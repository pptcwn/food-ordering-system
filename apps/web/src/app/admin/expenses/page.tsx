...
export default function ExpensesPage() {
  ...
  return <div className="min-h-screen bg-slate-50 p-4 pb-20 md:p-6"><div className="mx-auto max-w-6xl space-y-5">
    <header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 pb-4"><div><h1 className="text-xl font-black text-slate-900">รายจ่ายและภาษี</h1><p className="text-xs text-slate-500">เก็บหลักฐานรายจ่าย แยกตามวันและสรุปรายเดือน</p></div><div className="flex gap-2"><select value={branchId} onChange={(event) => setBranchId(event.target.value)} className="rounded-xl border bg-white px-3 py-2 text-sm font-bold">{managedBranches.map((branch: any) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select>{activeSection === 'EXPENSES' && <input type="month" value={month} onChange={(event) => setMonth(event.target.value)} className="rounded-xl border bg-white px-3 py-2 text-sm font-bold" />}</div></header>
